...
$fish_info = $this->db->get_where('fish_type',array('fish_type_id'=>$fish_id))->result();
          


if(!isset($fish_info) || empty($fish_info))
{
		$name='';
}
else
{
	$name=$fish_info[0]->name;
}
...
{
	$tname=$fish_info[0]->tname;
}

$final_category='crop';


echo form_open('fisheries/store_fish_type/'.$selected_fish.'/'.$return_to,array('class'=>'form','enctype'=>'multipart/form-data'));
echo "<table>";
...
if(!empty($fish_info) )
{
echo '<input type="file" style="width:auto;" name="icon[]"  class="upload"  />';
}
else
{
...
{
echo '<tr>';
echo '<td>Previous image</td><td>';
echo "<img style='max-width:150px;' src='".base_url()."assets/uploaded/img/".$fish_info[0]->icon."'>";
