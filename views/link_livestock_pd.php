...
echo form_open('livestock_pd/link_livestock_pd_next/'.$selected_pd,array('class'=>'form','enctype'=>'multipart/form-data'));
echo "<table>";

$this->db->from('livestock');
$this->db->select(array('livestock_id','name','tname','icon'));
...
foreach($result_livestock as $livestock_record)
{


echo "<div id='link_box'>
<img  src='".base_url()."assets/uploaded/img/thumb_".$livestock_record->icon."' />

<input type='radio' name='livestock_id[]' id='livestock_id' value='".$livestock_record->livestock_id."'/>";

echo $livestock_record->name.'/ '.$livestock_record->tname;
