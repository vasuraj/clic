...
$selected_agri_pd=$this->uri->segment(3);
if($selected_agri_pd!='' || $selected_agri_pd!=null)
{
$selected_agri_pd_info=$this->model_agri_pd->get_agri_pd($selected_agri_pd);


foreach ($selected_agri_pd_info as $value) {
	


	 	$this->$agri_pd_id =$value->agri_pd_id;
        $this->$name =$value->name;
        $this->$tname = $value->tname;
        $this->$intro = $value->intro;
        $this->$soils=$value->soils;
        $this->$varieties= $values->$varieties;
        $this->$seasonality = $values->$seasonality;
        $this->$seed_quantity = $values->$seed_quantity;
        $this->$seed_treatment = $values->$seed_treatment;
        $this->$sowing  = $values->$sowing;
        $this->$water_management  = $values->$water_management;
        $this->$weed_management  = $values->$weed_management;
        $this->$nutrient_management  = $values->$nutrient_management;
        $this->$harvesting  = $values->$harvesting;
        $this->$economics  = $values->$economics;
        $this->$other_info  = $values->$other_info;
...
echo form_open('agri_pd/store_agri_pd/'.$selected_agri_pd,array('class'=>'form','enctype'=>'multipart/form-data'));
echo "<table>";
...
echo '<input type="file" style="width:auto;" name="slide_media_files[]"  class="upload validate[required]"  />';
